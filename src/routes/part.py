...
@router.post("/create")
async def _create(body: PartCreate, db_session: AsyncSession = Depends(get_session)):
    return await create(body, db_session)


@router.put("/update")
async def _update(body: PartFromDB, db_session: AsyncSession = Depends(get_session)):
    return await update(body, db_session)


@router.put("/filter")
async def _filter(body: PartFromFilter, db_session: AsyncSession = Depends(get_session)):
